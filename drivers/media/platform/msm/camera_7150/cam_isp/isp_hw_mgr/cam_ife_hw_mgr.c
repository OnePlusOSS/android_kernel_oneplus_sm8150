...
//	list_for_each_entry(hw_mgr_res, &ife_ctx->res_list_ife_csid, list) {  //test_jitter related changes
	hw_mgr_res = list_first_entry(&ife_ctx->res_list_ife_csid,struct cam_ife_hw_mgr_res, list);
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i])
				continue;
